...
export class AnimationManager {
  animations: Map<HTMLElement, AnimationItem>
  isPaused: boolean
  ...
  addElement(element: HTMLElement, delay: number, currentTime: number, duration = 0.5) {
  ...
  setAnimationTimeout(anim: AnimationItem, element: HTMLElement) {
  ...
  clearCompletion(element: HTMLElement) {
    if (element.nodeName == 'LI') {
  ...
  play(currentTime: number, playbackSpeed: number) {
  ...
  refresh(currentTime: number, playbackSpeed: number, isWordByWord: boolean) {
    this.animations.forEach((anim, element) => {
      anim.remainingDelay = (anim.delay - currentTime) * playbackSpeed
      if (
